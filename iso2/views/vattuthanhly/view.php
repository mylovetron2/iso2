        ...
        <div class="lg:col-span-2 space-y-4">
            ...
            <div class="card bg-white rounded-lg shadow-md overflow-hidden">
                <div class="bg-yellow-500 text-gray-900 px-4 py-3 flex justify-between items-center">
                    <h5 class="text-lg font-bold"><i class="fas fa-exchange-alt mr-2"></i>Lịch sử thay đổi số lượng</h5>
                </div>
                <div class="p-4">
                    <?php if (empty($lichSuList)): ?>
                        <p class="text-gray-500 text-center py-3">
                            <i class="fas fa-inbox mr-2"></i>Chưa có lịch sử thay đổi
                        </p>
                    <?php else: ?>
                        <div class="bg-yellow-50 border border-yellow-200 rounded px-4 py-2 mb-3">
                            <i class="fas fa-exchange-alt text-yellow-600 mr-2"></i>
                            <strong>Tổng số thay đổi:</strong> <?php echo count($lichSuList); ?> lần
                        </div>
                        <div class="overflow-auto max-h-[400px]">
                            <table class="w-full border-collapse border border-gray-300 text-sm">
                                <thead class="bg-yellow-500 text-gray-900 sticky top-0">
                                    <tr>
                                        <th class="border border-gray-300 px-3 py-2 text-center">#ID</th>
                                        <th class="border border-gray-300 px-3 py-2 text-center"><i class="fas fa-tags mr-1"></i>Loại</th>
                                        <th class="border border-gray-300 px-3 py-2 text-right"><i class="fas fa-arrow-left mr-1"></i>SL trước</th>
                                        <th class="border border-gray-300 px-3 py-2 text-right"><i class="fas fa-exchange-alt mr-1"></i>Thay đổi</th>
                                        <th class="border border-gray-300 px-3 py-2 text-right"><i class="fas fa-arrow-right mr-1"></i>SL sau</th>
                                        <th class="border border-gray-300 px-3 py-2"><i class="fas fa-user mr-1"></i>Người</th>
                                        <th class="border border-gray-300 px-3 py-2 text-center"><i class="fas fa-clock mr-1"></i>Ngày</th>
                                        <th class="border border-gray-300 px-3 py-2"><i class="fas fa-comment-dots mr-1"></i>Lý do</th>
                                    </tr>
                                </thead>
                                <tbody class="bg-white">
                                    <?php foreach ($lichSuList as $index => $ls): ?>
                                    <tr class="<?php echo $index % 2 == 0 ? 'bg-gray-50' : 'bg-white'; ?> hover:bg-yellow-50">
                                        <td class="border border-gray-300 px-3 py-2 text-center font-bold"><?php echo $ls['id']; ?></td>
                                        <td class="border border-gray-300 px-3 py-2 text-center">
                                            <?php 
                                            $loaiBadge = [
                                                'nhap' => 'bg-green-500 text-white',
                                                'xuat' => 'bg-red-500 text-white',
                                                'dieu_chinh' => 'bg-blue-500 text-white',
                                                'thanh_ly' => 'bg-yellow-500 text-gray-900'
                                            ];
                                            $loaiIcons = [
                                                'nhap' => 'fa-plus-circle',
                                                'xuat' => 'fa-minus-circle',
                                                'dieu_chinh' => 'fa-edit',
                                                'thanh_ly' => 'fa-recycle'
                                            ];
                                            $badgeClass = $loaiBadge[$ls['loai_thaydoi']] ?? 'bg-gray-500 text-white';
                                            $iconClass = $loaiIcons[$ls['loai_thaydoi']] ?? 'fa-circle';
                                            
                                            $loaiText = '-';
                                            if ($ls['loai_thaydoi'] == 'nhap') {
                                                $loaiText = 'Nhập';
                                            } elseif ($ls['loai_thaydoi'] == 'xuat') {
                                                $loaiText = 'Xuất';
                                            } elseif ($ls['loai_thaydoi'] == 'dieu_chinh') {
                                                $loaiText = 'Điều chỉnh';
                                            } elseif ($ls['loai_thaydoi'] == 'thanh_ly') {
                                                $loaiText = 'Thanh lý';
                                            } else {
                                                $loaiText = htmlspecialchars($ls['loai_thaydoi']);
                                            }
                                            ?>
                                            <span class="<?php echo $badgeClass; ?> px-2 py-1 rounded text-xs">
                                                <i class="fas <?php echo $iconClass; ?> mr-1"></i><?php echo $loaiText; ?>
                                            </span>
                                        </td>
                                        <td class="border border-gray-300 px-3 py-2 text-right text-gray-600"><?php echo number_format($ls['soluong_truoc'] ?? 0, 0); ?></td>
                                        <td class="border border-gray-300 px-3 py-2 text-right">
                                            <?php 
                                            $change = floatval($ls['soluong_thaydoi'] ?? 0);
                                            $color = $change >= 0 ? 'text-green-600' : 'text-red-600';
                                            $icon = $change >= 0 ? 'fa-arrow-up' : 'fa-arrow-down';
                                            ?>
                                            <strong class="<?php echo $color; ?>">
                                                <i class="fas <?php echo $icon; ?> mr-1"></i>
                                                <?php echo $change >= 0 ? '+' : ''; ?><?php echo number_format($change, 0); ?>
                                            </strong>
                                        </td>
                                        <td class="border border-gray-300 px-3 py-2 text-right"><strong class="text-blue-600"><?php echo number_format($ls['soluong_sau'] ?? 0, 0); ?></strong></td>
                                        <td class="border border-gray-300 px-3 py-2">
                                            <i class="fas fa-user-circle text-blue-600 mr-1"></i>
                                            <?php echo htmlspecialchars($ls['nguoi_thuchien'] ?? '-'); ?>
                                        </td>
                                        <td class="border border-gray-300 px-3 py-2 text-center">
                                            <small class="text-gray-600">
                                                <?php echo !empty($ls['ngay_thuchien']) ? date('d/m/Y H:i', strtotime($ls['ngay_thuchien'])) : '-'; ?>
                                            </small>
                                        </td>
                                        <td class="border border-gray-300 px-3 py-2">
                                            <small class="text-gray-600 italic"><?php echo htmlspecialchars($ls['lydothaydo'] ?? '-'); ?></small>
                                        </td>
                                    </tr>
                                    <?php endforeach; ?>
                                </tbody>
