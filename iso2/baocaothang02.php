...
";
	...
	// ISO2: Chỉ áp dụng lọc tạm dừng nếu bảng hososcbd_tamdung tồn tại, tránh lỗi SQL làm rỗng kết quả
	$exclude_tamdung_base = "";
	try {
		$tamdungCheck = @mysql_query("SHOW TABLES LIKE 'hososcbd_tamdung'");
		if ($tamdungCheck && @mysql_num_rows($tamdungCheck) > 0) {
			$exclude_tamdung_base = "AND hoso NOT IN (SELECT hoso FROM hososcbd_tamdung WHERE trangthai='dang_tam_dung' AND id IN (SELECT MAX(id) FROM hososcbd_tamdung GROUP BY hoso))";
		}
	} catch (\Throwable $e) {
		$exclude_tamdung_base = "";
	}
